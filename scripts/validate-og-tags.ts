...
function validateRoute(route: {
  ...
  if (route.title && route.title.length > 0) {
    ...
    if (route.title.length > 60) {
      issues.push(
        `Title too long (${route.title.length} chars, recommended < 60)`,
      );
    }
  } else {
  ...
  if (route.description && route.description.length > 0) {
    ...
    if (route.description.length > 160) {
      issues.push(
        `Description too long (${route.description.length} chars, recommended < 160)`,
      );
    }
    if (route.description.length < 50) {
      issues.push(
        `Description too short (${route.description.length} chars, recommended > 50)`,
      );
    }
  } else {
  ...
  const valid =
    issues.filter((i) => !i.includes("(will use default)")).length === 0;
  ...
  results.forEach((result) => {
    ...
    if (result.issues.length > 0) {
      result.issues.forEach((issue) => {
        const icon = issue.includes("will use default")
          ? "  ℹ️"
          : issue.includes("too")
            ? "  ⚠️"
            : "  ❌";
