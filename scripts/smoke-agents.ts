...
async function runSmokeTests(): Promise<SmokeTestReport> {
  ...
  for (let i = 0; i < 3; i++) {
    const result = await testAgent('doc', '/api/agents/generate/doc', docPayload);
    docTests.push(result);
    console.log(`  Run ${i + 1}: ${result.latency_ms}ms ${result.status === 200 ? '✅' : '❌'}`);
  }

  console.log('\n🎨 Testing Design Agent (3 runs)...');
  for (let i = 0; i < 3; i++) {
    const result = await testAgent('design', '/api/agents/generate/design', designPayload);
  ...
  for (let i = 0; i < 3; i++) {
    const result = await testAgent('advisor', '/api/agents/generate/advisor', advisorPayload);
  ...
  const calcBfsAvg = (tests: TestResult[]) => {
    const bfsScores = tests.filter(t => t.bfs !== undefined).map(t => t.bfs!);
  ...
  const passed = allTests.filter(t => t.status === 200 && t.latency_ms < LATENCY_THRESHOLD_MS).length;
...
async function main() {
  try {
    ...
    console.log(`\n⏱️  Latency Results:`);
    console.log(`  Doc Agent:     ${report.doc_avg_latency}ms ${report.doc_avg_latency < LATENCY_THRESHOLD_MS ? '✅' : '❌'}`);
    console.log(`  Design Agent:  ${report.design_avg_latency}ms ${report.design_avg_latency < LATENCY_THRESHOLD_MS ? '✅' : '❌'}`);
    console.log(`  Advisor Agent: ${report.advisor_avg_latency}ms ${report.advisor_avg_latency < LATENCY_THRESHOLD_MS ? '✅' : '❌'}`);
    console.log(`  Overall Avg:   ${report.summary.avg_latency}ms`);
    console.log(`  Threshold:     ${LATENCY_THRESHOLD_MS}ms`);

    console.log(`\n🎯 BFS Scores:`);
    console.log(`  Doc Agent:     ${report.bfs.doc} ${report.bfs.doc >= BFS_THRESHOLD ? '✅' : '❌'}`);
    console.log(`  Design Agent:  ${report.bfs.design} ${report.bfs.design >= BFS_THRESHOLD ? '✅' : '❌'}`);
    console.log(`  Advisor Agent: ${report.bfs.advisor} ${report.bfs.advisor >= BFS_THRESHOLD ? '✅' : '❌'}`);
