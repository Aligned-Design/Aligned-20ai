...
export class AutoPlanGenerator {
  ...
  async generateMonthlyPlan(
    ...
    const monthStart = new Date(planMonth.getFullYear(), planMonth.getMonth(), 1);
    const monthEnd = new Date(planMonth.getFullYear(), planMonth.getMonth() + 1, 0);
    ...
    const insights = await advisorEngine.generateInsights({
      ...
      historicalMetrics: formattedMetrics.slice(0, Math.floor(formattedMetrics.length / 2)),
      goals: [],
      userFeedback: []
    });
    ...
    const formats = forecast.recommendations?.topFormats || ['video', 'carousel', 'image'];
    const bestTimes = forecast.recommendations?.bestTimes || ['9:00 AM', '1:00 PM', '7:00 PM'];
    ...
    const recommendedPostCount = forecast.predictions?.optimalPostCount || Math.ceil(daysInMonth / 3);
  ...
  private extractTopicsFromInsights(insights: unknown[]): string[] {
    ...
    topicInsights.forEach(insight => {
      if (insight.suggestions) {
        insight.suggestions.forEach((suggestion: string) => {
          ...
          if (suggestion.includes('behind-the-scenes')) topics.add('behind-the-scenes');
          if (suggestion.includes('tips')) topics.add('tips');
          if (suggestion.includes('user-generated')) topics.add('user-generated-content');
    ...
    return topicsArray.length > 0 ? topicsArray.slice(0, 5) : ['behind-the-scenes', 'tips', 'user-generated-content'];
  }
  ...
  private createContentCalendar(
    ...
    for (let week = 1; week <= weeksInMonth; week++) {
      const weekTopics = topics.length > 0 ? [topics[(week - 1) % topics.length]] : ['general'];
      const weekPlatforms = this.selectPlatformsForWeek(week, platforms, postsPerWeek);
   ...
   */
  private selectPlatformsForWeek(week: number, allPlatforms: string[], postsPerWeek: number): string[] {
