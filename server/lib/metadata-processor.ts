...
export function categorizeByContent(filename: string, mimeType: string, metadata: MediaMetadata): MediaCategory {
  const ext = filename.toLowerCase().split('.').pop() || '';
  
  // Video files
  if (mimeType.startsWith('video/') || ['mp4', 'mov', 'webm', 'avi'].includes(ext)) {
    return 'videos';
  }
  ...
  if (['psd', 'sketch', 'fig', 'pdf'].includes(ext) ||
...
export function generateSEOMetadata(asset: MediaAsset, context: 'web' | 'google_business' | 'social' = 'web'): {
  altText: string;
  title: string;
  description: string;
} {
