...
export async function recoverPublishingJobs(): Promise<void> {
  try {
    ...
    if (scheduledError) throw new Error(`Failed to fetch scheduled jobs: ${scheduledError.message}`);
    ...
    if (processingError) throw new Error(`Failed to fetch processing jobs: ${processingError.message}`);

    const totalJobs = pendingJobs.length + scheduledJobs.length + processingJobs.length;
    ...
    console.log(`✅ Job recovery complete. ${totalJobs} jobs restored to queue.`);
  } catch (error) {
...
export async function verifyJobSync(brandId: string): Promise<{
  ...
  try {
    const { total: dbJobCount } = await publishingDBService.getJobHistory(brandId, 1000, 0);
    const queueJobs = publishingQueue.getJobsByBrand(brandId);

    const status =
      dbJobCount === queueJobs.length ? 'healthy' : 'warning';
