...
export async function calculateEnhancedBFS(
  ...
  try {
    toneClassification = await toneClassifier.classifyTone(combinedText, toneTarget?.primary_tone);
  } catch (error) {
  ...
  if (toneAlignmentScore < 0.7) {
    issues.push(`Tone does not match brand personality (score: ${toneAlignmentScore.toFixed(2)})`);
  }
...
function calculateToneAlignmentMatch(
  ...
  const similarity = toneClassifier.compareTones(detectedTone, toneTarget.primary_tone);
...
export function getToneRecommendations(
  ...
  if (issues.some((issue) => issue.includes('Tone'))) {
    recommendations.push('Review tone keywords and personality traits in your brand guide');
  }
