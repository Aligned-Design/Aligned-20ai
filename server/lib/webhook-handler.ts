...
export class WebhookHandler {
  ...
  public verifySignature(
    ...
    if (!config) {
      console.warn(`[Webhook] Unknown provider for signature verification: ${provider}`);
      return false;
    }
   ...
   */
  public async handleEvent(request: WebhookHandlerRequest): Promise<WebhookHandlerResponse> {
    const startTime = Date.now();

    try {
      // Step 1: Check for idempotency (duplicate detection)
      const existingEvent = await webhookEvents.getByIdempotencyKey(request.idempotencyKey);
      if (existingEvent) {
        console.log(`[Webhook] Duplicate event detected (idempotency_key: ${request.idempotencyKey})`);
        return {
          success: true,
          eventId: existingEvent.id,
          status: existingEvent.status as 'pending' | 'processing' | 'delivered' | 'failed' | 'dead_letter',
      ...
      console.log(`[Webhook] Event processing completed (id: ${event.id}, duration: ${duration}ms)`);

      return {
        success: delivered,
        eventId: event.id,
        status: delivered ? 'delivered' : 'pending',
        message: delivered ? 'Event processed successfully' : 'Event queued for retry',
      };
    } catch (error) {
  ...
  private async deliverEvent(eventId: string, event: any): Promise<boolean> {
    try {
      ...
      await webhookEvents.markDeadLetter(eventId, 'Max retry attempts exceeded');
      return false;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(`[Webhook] Delivery error for event ${eventId}: ${errorMessage}`);
      ...
      await webhookAttempts.create({
        ...
        status: 'failed',
        error: errorMessage,
        backoff_ms: calculateBackoffDelay(event.attempt_count, this.retryConfig),
      });
   ...
   */
  public async retryPendingEvents(maxAgeMinutes: number = 60): Promise<{ retried: number; failed: number }> {
    try {
      const candidates = await webhookEvents.getRetryPendingEvents(maxAgeMinutes);
      let retried = 0;
      let failed = 0;
      ...
      for (const event of candidates) {
        try {
          ...
          const eventData = {
            id: event.id,
            status: event.status as 'pending' | 'failed' | 'processing' | 'delivered' | 'dead_letter',
      ...
      console.log(`[Webhook] Retry batch complete: ${retried} retried, ${failed} failed`);
