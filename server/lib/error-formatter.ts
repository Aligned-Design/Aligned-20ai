  ...
  private formatError(error: Error | APIError | string, _includeStack = false): APIError {
    ...
    if (message.includes('Invalid or expired OAuth state')) {
      return {
        ...
        message: 'The OAuth authorization has expired or is invalid. Please try connecting again.',
        statusCode: 400,
    ...
    if (message.includes('Platform mismatch')) {
      return {
        ...
        message: 'The platform in the OAuth response did not match the requested platform.',
        statusCode: 400,
        details: { originalError: message },
        recoveryHints: ['Ensure you approved the correct platform', 'Try connecting again'],
    ...
    if (message.includes('Token exchange failed')) {
      return {
        ...
        statusCode: 500,
        details: { originalError: message },
        recoveryHints: ['Try connecting again', 'Contact support if problem persists'],
    ...
    if (message.includes('No refresh token available')) {
      return {
        ...
        message: 'Your connection has expired and cannot be refreshed. Please reconnect.',
        statusCode: 401,
        details: { originalError: message },
        recoveryHints: ['Reconnect your platform account', 'Check platform permission settings'],
    ...
    if (message.includes('Token refresh failed')) {
      return {
        code: ErrorCode.AUTH_EXPIRED,
        message: 'Failed to refresh your platform connection. Please reconnect.',
        statusCode: 401,
        details: { originalError: message },
    ...
    if (message.includes('Failed to get account info')) {
      return {
        ...
        message: 'Could not retrieve your account information from the platform.',
        statusCode: 500,
        details: { originalError: message },
        recoveryHints: ['Try connecting again', 'Verify your platform account is active'],
