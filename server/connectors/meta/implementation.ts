...
export class MetaConnector extends BaseConnector {
  ...
  constructor(
    ...
    this.clientId = config?.clientId || process.env.META_CLIENT_ID || '';
    this.clientSecret = config?.clientSecret || process.env.META_CLIENT_SECRET || '';
    const redirectUri = config?.redirectUri || `${process.env.APP_URL || 'http://localhost:3000'}/api/oauth/meta/callback`;
