...
export async function processBrandIntake(brandId: string) {
  ...
  console.log(`[Brand Crawler] Brand intake processing complete for ${brandId}`);
}
...
export async function generateVisualSummary(brandKit: any, uploadedAssets: any[]) {
  ...
  return {
    ...
    logo_urls: uploadedAssets.filter((a) => a.asset_type === 'logo').map((a) => a.file_url),
