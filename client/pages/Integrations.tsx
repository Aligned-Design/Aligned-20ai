...
import { PLATFORM_CONFIGS, PlatformConnection, PlatformProvider } from '@/types/integrations';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function Integrations() {
  ...
  const categories = [
    { id: 'all', label: 'All Platforms', count: Object.keys(PLATFORM_CONFIGS).length },
    { id: 'social', label: 'Social Media', count: Object.values(PLATFORM_CONFIGS).filter(p => p.category === 'social').length },
    { id: 'video', label: 'Video Platforms', count: Object.values(PLATFORM_CONFIGS).filter(p => p.category === 'video').length },
    { id: 'professional', label: 'Professional', count: Object.values(PLATFORM_CONFIGS).filter(p => p.category === 'professional').length },
    { id: 'email', label: 'Email Marketing', count: Object.values(PLATFORM_CONFIGS).filter(p => p.category === 'email').length },
    { id: 'web_blog', label: 'Web & Blog', count: Object.values(PLATFORM_CONFIGS).filter(p => p.category === 'web_blog').length },
    { id: 'reviews', label: 'Reviews', count: Object.values(PLATFORM_CONFIGS).filter(p => p.category === 'reviews').length },
    { id: 'ecommerce', label: 'E-Commerce', count: Object.values(PLATFORM_CONFIGS).filter(p => p.category === 'ecommerce').length },
  ];
    ...
    <div className="p-10 space-y-10">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filteredPlatforms.map(platform => {
          const platformConnections = getConnectionForPlatform(platform.provider);
          const isComingSoon = platform.tier === 'coming_soon';

          return (
            <Card key={platform.provider} className="group relative overflow-hidden">
              <CardHeader>
                <div className="flex items-start justify-between">
                  ...
                  {platform.tier !== 'coming_soon' && (
                    <Badge variant={platform.tier === 1 ? 'default' : 'secondary'}>
                      Tier {platform.tier}
                    </Badge>
                  )}
                </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-3 border-t border-border/50">
                  <p className="text-xs text-muted-foreground mb-2">Supported content:</p>
  ...
  const getStatusText = () => {
    if (connection.status === 'connected' && connection.token_expires_at) {
      ...
      if (daysUntilExpiry < 7) {
        return <span className="text-coral">Expires in {daysUntilExpiry}d</span>;
      }
      ...
      <div className="flex gap-2">
        ...
        <Dialog>
          <DialogTrigger asChild>
            <Button variant="outline" size="sm" className={connection.status === 'expired' ? 'flex-1' : 'w-full'}>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <div className="rounded-lg border border-border/50 p-3 space-y-1 text-xs">
                  ...
                  {connection.scopes && (
                    <div className="pt-2">
                      <span className="text-muted-foreground block mb-1">Permissions:</span>
                      <div className="flex flex-wrap gap-1">
