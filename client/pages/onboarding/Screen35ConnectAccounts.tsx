...
import { ArrowRight, Plus, X, CheckCircle2, AlertCircle, RefreshCw, Info, HelpCircle } from "lucide-react";
...
const ERROR_EXPLANATIONS: Record<string, { title: string; description: string; solution: string }> = {
  permission_denied: {
    title: "Permission Denied",
    description: "You declined to grant permissions to Aligned AI.",
    solution: "Click 'Retry' and make sure to accept all required permissions in the authorization window.",
  },
  token_expired: {
    title: "Session Expired",
    description: "The authorization session expired or timed out.",
    solution: "Click 'Retry' to start a fresh connection. The process usually takes less than 30 seconds.",
  },
  network_error: {
    title: "Network Error",
    description: "Unable to connect to the platform's servers.",
    solution: "Check your internet connection and try again. If the problem persists, the platform may be experiencing issues.",
  },
  invalid_credentials: {
    title: "Invalid Credentials",
    description: "The account credentials couldn't be verified.",
    solution: "Make sure you're logged into the correct account in your browser before trying again.",
  },
};
...
export default function Screen35ConnectAccounts() {
  ...
  const handleConnect = async (id: string) => {
    ...
    setPlatforms(platforms.map((p) => (p.id === id ? { ...p, error: undefined } : p)));
    ...
    if (success) {
      setPlatforms(platforms.map((p) => (
        p.id === id 
          ? { ...p, connected: true, username: `@user_${id}`, error: undefined } 
          : p
      )));
    } else {
      // Simulate error
      const errorTypes = ["permission_denied", "token_expired", "network_error"];
      ...
      <div className="max-w-3xl mx-auto pt-6">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-600 font-medium">
            Link your social platforms to start publishing from Aligned
          </p>
                  ...
                  <div className="px-3 py-2 bg-green-100 rounded-lg border border-green-200">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-green-800 font-medium truncate">{platform.username}</p>
                      </div>
                    </div>
                  </div>
                  <button
                  ...
                  <div className="px-3 py-2 bg-red-100 rounded-lg border border-red-200">
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div className="flex-1">
                        <p className="text-xs text-red-700 font-bold">{platform.error.message}</p>
                        <p className="text-xs text-red-600 mt-1">
                          {ERROR_EXPLANATIONS[platform.error.reason]?.description}
                        </p>
                      </div>
                    </div>
                      ...
                      <DialogContent className="max-w-md">
                        <DialogHeader>
                          ...
                          <DialogDescription className="text-left space-y-3 pt-2">
                            <div>
                              <p className="text-sm font-bold text-slate-900 mb-1">What happened:</p>
                              <p className="text-sm text-slate-600">
                                {ERROR_EXPLANATIONS[platform.error.reason]?.description}
                              </p>
                            </div>
                            <div>
                              <p className="text-sm font-bold text-slate-900 mb-1">How to fix it:</p>
                              <p className="text-sm text-slate-600">
                                {ERROR_EXPLANATIONS[platform.error.reason]?.solution}
                              </p>
                            </div>
                          </DialogDescription>
                        </DialogHeader>
              ...
              {hasErrors ? (
                <>
                  ...
                  <p className="text-xs text-amber-800">
                    You can retry connections now or set them up later from Settings → Integrations. 
                    You can still use Aligned to plan and approve content without platform connections.
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-xs text-blue-800">
                    You can add or change platforms anytime from Settings. Starting without connections? 
                    No problem—use Aligned to plan and approve content, then manually post if needed.
                  </p>
                </>
              )}
            </div>
