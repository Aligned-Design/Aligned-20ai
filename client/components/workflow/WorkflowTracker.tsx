        ...
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            ...
            <Badge variant="outline" className={cn(
              workflow.status === 'completed' ? 'bg-green-50 text-green-700' :
              workflow.status === 'cancelled' ? 'bg-red-50 text-red-700' :
          ...
          <div className="text-right">
            <p className="text-sm text-gray-600">{completedSteps} of {totalSteps} steps</p>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-3 bg-gray-50 rounded-lg">
          <div>
            ...
            <Badge variant={
              workflow.metadata.priority === 'high' ? 'destructive' :
              workflow.metadata.priority === 'medium' ? 'default' : 'secondary'
            }>
          ...
          <div>
            ...
            <p className="text-sm">{new Date(workflow.startedAt).toLocaleDateString()}</p>
          </div>
        </div>
...
function WorkflowStepCard({ step, index, isLast, canTakeAction, onAction }: WorkflowStepCardProps) {
  const getStepStatusIcon = (status: WorkflowStepInstance['status']) => {
    switch (status) {
      case 'completed':
        return <CheckCircle className="h-5 w-5 text-green-500" />;
        ...
        <div className="flex-1 min-w-0">
          ...
          {step.assignedTo && (
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm text-gray-600">Assigned to {step.assignedTo}</span>
              {step.assignedAt && (
          ...
          {step.comments.length > 0 && (
            <div className="mb-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium">Comments ({step.comments.length})</span>
              </div>
