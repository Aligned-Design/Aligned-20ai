...
import { BrandIntakeFormData, PLATFORMS, POST_FREQUENCIES, CONTENT_TYPES } from '@/types/brand-intake';
...
export default function Section4ContentPreferences({ data, onChange, errors }: Section4Props) {
  ...
  const removeHashtag = (hashtag: string) => {
    const current = data.hashtagsToInclude || [];
    onChange('hashtagsToInclude', current.filter((h) => h !== hashtag));
  };
  ...
  const removeCompetitor = (competitor: string) => {
    const current = data.competitorsOrInspiration || [];
    onChange('competitorsOrInspiration', current.filter((c) => c !== competitor));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={data.postFrequency || ''} onValueChange={(value) => onChange('postFrequency', value)}>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Label htmlFor="competitors">Competitors or Inspiration Brands</Label>
