...
interface ApprovalOption {
  type: 'approve' | 'approve_with_suggestions' | 'request_changes' | 'ask_question';
  label: string;
  description: string;
...
export function CollaborativeApprovalFlow({
  ...
  const [selectedOption, setSelectedOption] = useState<ApprovalOption['type'] | null>(null);
      ...
      <CardContent className="p-6">
        ...
        {!selectedOption ? (
            ...
            {approvalOptions.map((option) => (
              <button
                ...
                className={cn(
                  ...
                  'hover:shadow-md',
                  option.color.includes('green') && 'border-green-200 hover:border-green-500',
                  option.color.includes('amber') && 'border-amber-200 hover:border-amber-500',
                  option.color.includes('red') && 'border-red-200 hover:border-red-500',
                  option.color.includes('blue') && 'border-blue-200 hover:border-blue-500'
                )}
              >
                <div className={cn(
                  'flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center',
                  option.color.includes('green') && 'bg-green-100 text-green-600',
                  option.color.includes('amber') && 'bg-amber-100 text-amber-600',
                  option.color.includes('red') && 'bg-red-100 text-red-600',
                  option.color.includes('blue') && 'bg-blue-100 text-blue-600'
                )}>
                  {option.icon}
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={cn(
                  ...
                  selectedOption === 'approve' && 'bg-green-100 text-green-600',
                  selectedOption === 'approve_with_suggestions' && 'bg-amber-100 text-amber-600',
                  selectedOption === 'request_changes' && 'bg-red-100 text-red-600',
                  selectedOption === 'ask_question' && 'bg-blue-100 text-blue-600'
                )}>
                  {approvalOptions.find(o => o.type === selectedOption)?.icon}
                </div>
                <div>
                  <h3 className="font-bold text-slate-900">
                    {approvalOptions.find(o => o.type === selectedOption)?.label}
                  </h3>
                  <p className="text-sm text-slate-600">
                    {approvalOptions.find(o => o.type === selectedOption)?.description}
                  </p>
                </div>
              </div>
              <Button
            ...
            {/* Comment Box */}
            {approvalOptions.find(o => o.type === selectedOption)?.requiresComment ? (
              <div>
                <Textarea
                  value={comment}
