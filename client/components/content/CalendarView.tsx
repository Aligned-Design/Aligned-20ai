...
export function CalendarView({ events, onEventMove, onEventClick, filter, onFilterChange }: CalendarViewProps) {
  ...
  const getEventsForDate = (date: Date) => {
    ...
    return events.filter(event => 
      event.scheduledAt.startsWith(dateStr)
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
        ...
        <select
          ...
          onChange={(e) => {
            const platforms = Array.from(e.target.selectedOptions, option => option.value) as PostModel['platform'][];
        ...
        <select
          ...
          onChange={(e) => {
            const statuses = Array.from(e.target.selectedOptions, option => option.value) as PostModel['status'][];
